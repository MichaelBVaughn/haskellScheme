...
data ClosureState = ClsSt [Symbol] SExpr Lib.State deriving (Show)

data SList a = Cons (SList a) (SList a) | Atom a | Nil deriving (Show)
 
type State = Symbol -> SList AtomicValue

type EvalContext = (Lib.State, SExpr)

type ErrorContext = Either String EvalContext

type Func = Lib.State -> SExpr -> SExpr -> ErrorContext
-- Parser definition
...
validateLambdaSyntax :: Lib.State -> SExpr -> ErrorContext
...
lambdaEval :: Lib.State -> SExpr -> ErrorContext
...
isApplicable :: Lib.State  -> SExpr -> Bool
...
closureSaturate :: Lib.State -> ClosureState -> SExpr -> ErrorContext
...
argSaturate :: Lib.State -> SExpr -> SExpr -> ErrorContext
...
fxnSaturateI :: Lib.State -> Func -> SExpr -> ErrorContext
...
fxnSaturate :: Lib.State -> SExpr -> SExpr -> ErrorContext
...
apply :: Lib.State  -> SExpr -> SExpr -> ErrorContext
...
evalApplicable :: Lib.State -> SExpr -> ErrorContext
...
evalAtom :: Lib.State -> AtomicValue -> ErrorContext
...
eval :: Lib.State -> SExpr -> ErrorContext
